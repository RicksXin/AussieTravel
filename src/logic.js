...
export function eventInstant(day,event){if(!event.time)return null;return new Date(`${day.date}T${event.time}:00${day.offset}`)}
export function nextEvent(days,now,done={}){return days.flatMap(day=>day.events.map(event=>({day,event,at:eventInstant(day,event)}))).filter(x=>x.at&&x.at>=now&&!done[x.event.id]&&!x.event.optional).sort((a,b)=>a.at-b.at)[0]||null}
...
export function ics(days){const escape=s=>String(s).replace(/\\/g,'\\\\').replace(/\r?\n/g,'\\n').replace(/,/g,'\\,').replace(/;/g,'\\;');const stamp=d=>d.toISOString().replace(/[-:]/g,'').replace(/\.\d{3}/,'');const lines=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//AussieTravel//CN','CALSCALE:GREGORIAN'];for(const day of days)for(const e of day.events){const at=eventInstant(day,e);if(!at||e.optional)continue;lines.push('BEGIN:VEVENT',`UID:${e.id}@aussie-travel`,`DTSTAMP:${stamp(new Date())}`,`DTSTART:${stamp(at)}`,`DTEND:${stamp(new Date(+at+30*60000))}`,`SUMMARY:${escape(e.title)}`,`DESCRIPTION:${escape('原计划时间；请按实际预订核对。日历显示为30分钟占位，并非实际活动时长。'+e.note)}`,`LOCATION:${escape(e.place||day.city)}`,'BEGIN:VALARM','TRIGGER:-PT30M','ACTION:DISPLAY',`DESCRIPTION:${escape(e.title)}`,'END:VALARM','END:VEVENT')}lines.push('END:VCALENDAR');return lines.map(foldLine).join('\r\n')+'\r\n'}
